...
*Маша.* Проектировали мы как-то систему профессий для RPG игры. У нас там был базовый класс Эльф, еще были Эльфы-Воины, Эльфы-Лучники, Эльфы-Арбалетчики и т.д. В игре может понадобиться целая армия Эльфов. Давай предположим, что все профессии имеют одинаковое поведение по силе, скорости и здоровью, но отличаются видом оружия. 
...
*Маша.* Нужно использовать Прототип, а затем модифицировать тип оружия: (2)
```c#
public abstract class Elf
{
    public int Power { get; } = 100;
    public int Health { get; } = 100;
    // SomeProperty может состоять из кучи других объектов, которые создавать заново накладно 
    public SomeProperty SomeProperty { get; } = new Someproperty();
...
public class ElfWarrior : Elf
{
    ...
    public override Elf Clone()
    {
        ...
        //Получаем ElfWarrior со свойственным ему оружием
        clone.Weapon = new NecessaryWeapon(); 
        return clone;
    }
}
...
public class ElfPixi : Elf
{
    ...
    {
         Elf clone = base.Clone(); 
         //Получаем ElfPixi со свойственным ему оружием
         clone.Weapon = new NecessaryWeapon(); 
         return clone;
    }
}
