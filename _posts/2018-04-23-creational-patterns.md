...
*Саша.* А вот взять к пример Абстрактную фабрику и Фабричный метод. По сути ведь вызов конструктора, порождающего другой объект, просто перемещается из одного класса в другой. И что же здесь за преимущество? Чем плохо на месте вызвать
```c#
